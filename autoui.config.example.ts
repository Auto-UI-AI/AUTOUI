...
export const autouiConfig: AutoUIConfig = {
  ...
  metadata: {
    ...
    description: 'Example AutoUI config for e-commerce store with React components and mock functions.',
  ...
  functions: {
    fetchProducts: {
      prompt: 'Fetch list of products filtered optionally by color, category, or query text.',
      params: {
        color: 'string (optional) — color filter',
        category: 'string (optional) — product category',
        q: 'string (optional) — search term',
      },
      callFunc: async ({ color, category, q }: { color?: string; category?: string; q?: string }) => {
        const all = [
    ...
    addToCart: {
      ...
      callFunc: async ({ productId, quantity }: { productId: string; quantity: number }) => {
  ...
  components: {
    ProductList: {
      ...
      exampleUsage: '<ProductList products={mockProducts} onAddToCart={(id)=>{}} />',
    },
