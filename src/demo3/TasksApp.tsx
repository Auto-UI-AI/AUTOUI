...
function updateTaskWithDraft(task: Task, patch: TaskDraft): Task {
  return {
    ...task,
    ...patch,
  };
  ...
  metadata: {
    ...
    description:
      "Task management demo app with tasks list, filters, stats, and task editor modal.",
  ...
  functions: {
    createTask: {
      prompt:
        "Create a new Task object from a TaskDraft by adding id and created_at timestamp.",
      params: {
      ...
      returns:
        "Task — full task with generated id and created_at fields ready to be added to the list.",
    },

    updateTask: {
      prompt:
        "Update an existing Task object using a TaskDraft patch, keeping id and created_at.",
      params: {
        task: "Task — current version of the task",
        patch:
          "TaskDraft — new values for fields like title, description, status, priority, due_date",
      },
      callFunc: (args: { task: Task; patch: TaskDraft }) => {
        return updateTaskWithDraft(args.task, args.patch);
      },
      returns:
        "Task — new merged task object after applying the provided patch.",
    },

    summarizeTasks: {
      prompt:
        "Given a list of tasks, compute basic statistics: total count and counts by status and priority.",
      params: {
    ...
    openTaskForm: {
        prompt: 'function opening the TaskForm on the main apps page so that the user would just input the data by himself',
  ...
  components: {
    ...
    TaskItem: {
      ...
      props: {
        ...
        onDelete:
          "function(task: Task) — delete this task from the list or ask for confirmation.",
        onStatusChange:
          "function(task: Task, nextStatus: Status) — change status of this task.",
      },
      defaults: {
        task: {
          id: "example-id",
      ...
      category: "tasks",
      exampleUsage:
        "<TaskItem task={t} onEdit={editTask} onDelete={deleteTask} onStatusChange={changeStatus} />",
    },

    // ---- Filters
    TaskFilters: {
      prompt:
        "Filter controls for tasks by status and priority. Use to narrow down the visible tasks.",
      props: {
        filters:
          "{ status: 'all' | 'todo' | 'in_progress' | 'done'; priority: 'all' | 'low' | 'medium' | 'high' } — current filters state.",
        onFilterChange:
          "function(next: TaskFiltersState) — update the filters based on user selection.",
      },
      defaults: {
    ...
    TaskStats: {
      prompt:
        "Statistics bar showing counts of tasks by status and possibly other metrics.",
      props: {
