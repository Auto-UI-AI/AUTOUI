  ...
  const example = `VALID EXAMPLE:
{
  ...
  return [
    ...
    '',
    'Respond with ONLY the plan JSON (no code fences, no commentary). NEVER RESPOND WITH ANYTHING ELSE THAN THE PLAN JSON. THE PLAN MUST ALWAYS BE RETURNED EVEN IF YOU DO NOT UNDERSTAND THE USER INTENT OR IF THE USER MESSAGE IS NOT RELATED TO THE APP DESCRIPTION. IF YOU DO NOT UNDERSTAND THE USER INTENT OR IF THE USER MESSAGE IS NOT RELATED TO THE APP DESCRIPTION, RETURN A PLAN WITH TEXT WHICH ASKS THE USER TO CLARIFY HIS INTENT OR WHICH ASKS THE USER TO ASK SOMETHING RELATED TO THE APP DESCRIPTION. REMEMBER THAT YOU HAVE TO RETURN A JSON LIKE THAT {}, NOT LIKE ```json {}``` OR ANYTHING ELSE.`,',
  ].join('\n');
}
